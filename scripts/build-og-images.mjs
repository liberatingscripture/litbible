 ...
 * owner's call.) The site emblem is deliberately absent there: two logos on
 * one card compete, and the wordmark line alone carries the brand.
 *
...
/**
 * The /apps share card. Text column on the left; on the right, the Android
 * gradient mark as a single home-screen tile, one step lighter than the
 * field (not the popover's near-ink tile, which would disappear against
 ...
 * (The card only shows Android — an earlier draft paired it with the iOS
 * icon, but the owner decided the share card stays single-icon while the
 * popover and in-page CTAs keep the platform pair.)
 */
function iconLayout() {
  const size = 300;
  const radius = 66; // ~22% of size, matching the app's own icon corners
...
function appsCardSVG() {
  const { x, y, size, radius } = iconLayout();
  // Text column runs from the card margin to a gutter left of the icon.
  const X = 90;
  const COLW = x - 60 - X;
...
<rect x="${x}" y="${y}" width="${size}" height="${size}" rx="${radius}" fill="#262E24"/>
...
async function renderAppsCard() {
  ...
  await sharp(Buffer.from(appsCardSVG()))
    .composite([{ input: androidMark, left: x + pad, top: y + pad }])
