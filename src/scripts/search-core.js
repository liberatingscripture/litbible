...
// enough (~810 KB raw, ~260 KB gzipped) to ship as plain text and scan
...
// cache. Matching is exact whole-word/phrase: both the query and the verse
// text are tokenized on non-alphanumerics (so hyphens and apostrophes act
// as word boundaries: "well" matches "well-known"), and multi-word queries
...
/**
 * Load /search/verses.json once. Resolves to the `verses` map
 * ({ bookKey: { chapter: ["verse 1 text", ...] } }) or null on failure
 ...
 * verse's text (offsets index the ORIGINAL text, for highlighting).
 * Occurrences don't overlap: matching resumes after each hit.
 */
function findTokenRuns(text, qTokens) {
  const words = [];
 ...
 */
export function searchVerses(verses, rawQuery, { bookKey = "" } = {}) {
  ...
  for (const bk of books) {
    ...
    for (const ch of chapterNums) {
      ...
      for (let vi = 0; vi < verseTexts.length; vi++) {
        ...
        const runs = findTokenRuns(text, qTokens);
